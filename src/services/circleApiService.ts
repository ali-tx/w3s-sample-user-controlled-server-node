...
interface CircleContractsResponse {
  data?: {
...
class CircleApiService {
  ...
  async fetchContractAddresses(): Promise<string[]> {
    try {
      ...
      const response: AxiosResponse<CircleContractsResponse> = await axios.get(
      ...
      if (response.status === 200 && response.data) {
        ...
        console.log('I am validateGas', validContracts);
        if (validContracts.length > 0) {
          validContracts.forEach((addr, i) => {
        ...
        const errorMsg = `Circle API returned status ${response.status}: ${
  ...
  async saveContractsToStorage(contractAddresses: string[]): Promise<void> {
    try {
      const storagePath = path.join(__dirname, '../deployed-contracts.json');
  ...
  async triggerSplit(contractAddress: string, amount: number): Promise<any> {
    try {
