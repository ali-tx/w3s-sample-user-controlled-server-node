...
class USDCWatcher {
  ...
  public eventsDetected: number = 0;
  public splitsExecuted: number = 0;
  public lastChecked: string | null = null;

  constructor(config: MonitoringConfig, circleService: any) {
    this.config = config;
    ...
    try {
      ...
      try {
        ...
        this.web3 = new Web3(
          new Web3HttpProvider(
            this.config.rpcUrl.replace('wss://', 'https://'),
          ),
        );
        const currentBlock = await this.web3.eth.getBlockNumber();
        this.lastBlock = currentBlock;
      ...
      if (useWebSocket) {
        ...
        this.catchupInterval = setInterval(() => {
          this.checkRecentTransfers().catch((error: Error) => {
        ...
        this.interval = setInterval(() => {
          this.checkForTransfers().catch((error: Error) => {
  ...
  async processTransfer(event: TransferEvent): Promise<void> {
    ...
    const transferId = `${event.transactionHash.slice(
      0,
      10,
    ...
    try {
      ...
      for (const contractAddr of contractAddresses) {
        ...
        subscription.on('data', (event: TransferEvent) => {
          ...
          this.processTransfer(event).catch((error: Error) => {
            console.log(
