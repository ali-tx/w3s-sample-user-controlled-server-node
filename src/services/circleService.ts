...
class CircleService {
  private config: CircleServiceConfig;

  constructor(config: CircleServiceConfig) {
    this.config = config;
  }
  ...
  async executeSplit(
    ...
    try {
      ...
      // Silent API call
      const response = await axios.post<CircleApiResponse>(
  ...
  async fetchContractsFromTransactions(hoursBack: number = 24): Promise<any[]> {
