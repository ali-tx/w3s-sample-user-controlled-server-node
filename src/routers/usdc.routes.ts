// backend/src/routes/usdc.routes.ts - Manual transfer monitoring only
...
router.post('/check-historical', async (req: Request, res: Response) => {
  try {
    ...
    const monitoringConfig = {
      ...
      contractAddresses: [] as string[], // Will be loaded from storage
    ...
    monitoringConfig.contractAddresses = await (
    ...
    if (monitoringConfig.contractAddresses.length === 0) {
      return res
        .status(400)
        .json({ error: 'No contracts available for monitoring' });
    }
...
router.post('/check-contract/:address', async (req: Request, res: Response) => {
  try {
    ...
    for (const event of events) {
      ...
      results.push({
        ...
        contractBalance: parseInt(balance as string) / 1000000,
...
router.post('/check-transfers', async (req: Request, res: Response) => {
  try {
...
router.post('/deploy-splitter', async (req: Request, res: Response) => {
  try {
    const { receiveWallet, feeWallet, name } = req.body;
    ...
    const compileResult = await (ContractCompiler as any).compileContract(
      sourceCode,
      name || 'USDC-Splitter'
    );
...
-----END PUBLIC KEY-----`;
    ...
    const err = error as Error;
    console.error('USDC Splitter deployment error:', err);
    res.status(500).json({
      success: false,
      error: err.message
    });
