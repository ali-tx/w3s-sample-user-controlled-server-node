...
async function fetchContracts() {
    let contractData = [];
    try {
        const circleApiService = await Promise.resolve().then(() => __importStar(require('./services/external/circleApiService.js')));
        const allContracts = [];
        let page = 1;
        const limit = 100;
        // eslint-disable-next-line no-constant-condition
        while (true) {
            const contractsResp = await circleApiService.get(`/v1/w3s/contracts?page=${page}&limit=${limit}`);
            logger_1.logger.info(`Contracts from API page ${page}:`, contractsResp.data);
        ...
        if (contracts.length > 0) {
            logger_1.logger.info('First contract:', contracts[0]);
        }
